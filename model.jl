...
@constraint(NPP, StorageBalance[f in fuels, p in period], Output[f,p] <= StorageUsed[f,p])
#We can only store as much fuel as we have capacities
@constraint(NPP, StorageCapacity[f in fuels, p in period], StorageUsed[f,p] <= StorageCapacity[f,p-1])
...
@Constraint(NPP, StorageBalanceUpdate[f in fuels, p in period], StorageUsed[f,p] == StorageUsed[f,p-1] + Input[f,p] - Output[f,p])
